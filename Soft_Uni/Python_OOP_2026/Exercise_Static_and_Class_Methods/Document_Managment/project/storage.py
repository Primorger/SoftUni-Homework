...
class Storage:
    ...
    def __edit_object(self, object_id: int, collection, *new_values) -> None:
        obj = self.__find_object(object_id, collection)
        if obj:
            obj.edit(*new_values)

    def __delete_object(self, object_id: int, collection) -> None:
        obj = self.__find_object(object_id, collection)
        if obj:
            collection.remove(obj)

    @staticmethod
    def __find_object(object_id: int, collection) -> Category | Topic | Document | None:
